...
@section('content')
    <div class="container py-8 space-y-6">
        <div class="flex items-center justify-between">
            <h1 class="text-2xl font-semibold text-gray-900">Theme Park Ticket #{{ $ticket->id }}</h1>
            <a href="{{ route('theme-park.tickets') }}" class="btn-secondary">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div class="card">
                ...
                <div class="card-body space-y-3">
                    <div class="flex justify-between"><span class="text-sm text-gray-500">Status</span><span class="text-sm font-medium">{{ ucfirst($ticket->ticket_status) }}</span></div>
                    <div class="flex justify-between"><span class="text-sm text-gray-500">Visit Date</span><span class="text-sm font-medium">{{ $ticket->visit_date }}</span></div>
                    <div class="flex justify-between"><span class="text-sm text-gray-500">Tickets</span><span class="text-sm font-medium">{{ $ticket->num_tickets }}</span></div>
